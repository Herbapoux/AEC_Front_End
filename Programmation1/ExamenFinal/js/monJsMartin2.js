...
$( "#boutonGo" ).click(function() {
	...
	for(let i=0; i<nomLettres.length; i++){
		...
		let MaLettreDiv = "<div id='imageTitre"+i+"' class='col-"+nbCol+" removeClass'>"+
							...
							"<img src='img/Letters/"+nomLettres[i]+"/"+nomLettres[i]+nbAleatoire+".jpg' id='lettreImage"+i+"' class='img-fluid' />"+"</a>"+"</div>"
		
		$("#conteneurLettres").append(MaLettreDiv);
		
		
//		let MaLettreDiv = "<div id='imageTitre"+i+"' class='col-"+nbCol+" removeClass'>"+
//							"<img src='img/Letters/"+nomLettres[i]+"/"+nomLettres[i]+nbAleatoire+".jpg' id='lettreImage"+i+"' class='img-fluid' onclick=\"fOuvreModal("+i+",'"+nomLettres[i]+"','"+nbAleatoire+"')\" />"+
//							"</div>"
//		
//		let image = $("<img />",{
//			id:"lettreImage"+i,
...
function fOuvreModal(LettrePosition,Lettre,LettreSelectionner){
	for(let j=1; j<6; j++){
		...
		$("#image"+j).attr("alt", "Lettre_"+Lettre+"_Position_"+LettrePosition);
	}
	$("#modalLettres").modal("show");
}
...
$('#carousel-example img').click(function() {
	...
	let aInformation = this.alt.split("_");
	
	$("#lettreImage"+aInformation[3]).attr("src", this.src);
